...
def outputids2words(id_list, vocab, article_oovs):
    ...
    for i in id_list:
        try:
            ...
            assertion_error = (
                "Error: model produced a word ID that isn't " +
                "in the vocabulary. This should not happen in baseline " +
                "(no pointer-generator) mode")
            assert article_oovs is not None, assertion_error
            ...
            try:
                ...
                value_error = (
                    'Error: model produced word ID %i which ' +
                    'corresponds to article OOV %i but this example only ' +
                    'has %i article OOVs') % (i, article_oov_idx,
                                              len(article_oovs))
