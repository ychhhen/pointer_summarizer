...
class Example(object):
    def __init__(self, article, abstract_sentences, vocab):
        ...
        if config.pointer_gen:
            ...
            # Get a verison of the reference summary where in-article OOVs
            # are represented by their temporary article OOV id
            abs_ids_extend_vocab = data.abstract2ids(abstract_words, vocab,
                                                     self.article_oovs)

            # Overwrite decoder target sequence so it uses the temp article
            # OOV ids
...
class Batch(object):
    ...
    def init_encoder_seq(self, example_list):
        ...
        # Pad the encoder input sequences up to the length of the
        # longest sequence
...
class Batcher(object):
    ...
    def __init__(self, data_path, vocab, mode, batch_size, single_pass):
        ...
        # Different settings depending on whether we're in single_pass
        # mode or not
        if single_pass:
            ...
            # only load one batch's worth of examples before bucketing;
            ...
            # 16 # num threads to fill example queue
            self._num_example_q_threads = 1
            # 4  # num threads to fill batch queue
            self._num_batch_q_threads = 1
            # 100 # how many batches-worth of examples to load into cache
            # before bucketing
            self._bucketing_cache_size = 1
        ...
        # Start a thread that watches the other threads and restarts
    ...
    def fill_example_queue(self):
        ...
        while True:
            try:
                # read the next example from file. article and abstract
                # are both strings.
            ...
            # if there are no more examples:
                ...
                if self._single_pass:
                    tf.logging.info(
                        "single_pass mode is on, so we've finished " +
                        "reading dataset. This thread is stopping.")
                    self._finished_reading = True
                    break
                else:
                    raise Exception(
                        "single_pass mode is off but the example generator " +
    ...
    def text_generator(self, example_generator):
        while True:
            ...
            try:
                # the article text was saved under the key 'article'
                # in the data files
                article_text = e.features.feature['article'].bytes_list.value[
                    0]
                # the abstract text was saved under the key 'abstract'
                # in the data files
