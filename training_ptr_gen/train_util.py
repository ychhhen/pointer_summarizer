...
def get_input_from_batch(batch, use_cuda):
    ...
    enc_padding_mask = Variable(torch.from_numpy(
        batch.enc_padding_mask)).float()
    ...
    if config.pointer_gen:
        enc_batch_extend_vocab = Variable(
            torch.from_numpy(batch.enc_batch_extend_vocab).long())
        # max_art_oovs is the max over all the article oov list in the batch
        if batch.max_art_oovs > 0:
            extra_zeros = Variable(
                torch.zeros((batch_size, batch.max_art_oovs)))
...
def get_output_from_batch(batch, use_cuda):
    ...
    dec_padding_mask = Variable(torch.from_numpy(
        batch.dec_padding_mask)).float()
    ...
    return (dec_batch, dec_padding_mask, max_dec_len, dec_lens_var,
            target_batch)
